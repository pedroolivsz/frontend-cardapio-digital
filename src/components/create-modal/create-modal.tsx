...
import { useFoodDataMutate } from "../../hooks/useFoodDataMutade";
...
const Input = ({label, value, updateValue}: InputProps) => {
    return (
        <>
            <label>{label}</label>
            <input value={value} onChange={event => updateValue(
                label === "price" ? Number(event.target.value) : event.target.value
            )}>
            </input>
        </>
    )
